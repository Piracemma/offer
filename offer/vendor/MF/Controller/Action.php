...
abstract class Action {
	...
	public function enviarEmail($operacao, $email, $codigo_verificacao = null) {

		$email_hsc = htmlspecialchars($email, ENT_QUOTES, 'UTF-8');

		$operacoes = [
			'validar_email' => [
				'Subject' => 'Validacao de E-mail - Offer',
				'Body' => 'Seu codigo para validar o e-mail &eacute;: <b>'. $codigo_verificacao .'</b>.',
				'AltBody' => 'Seu codigo para validar o e-mail é: '. $codigo_verificacao .'.',
			],
			'recuperar_senha' => [
				'Subject' => 'Recuperar Senha - Offer',
				'Body' => 'Seu codigo para alterar a senha &eacute;: <b>'. $codigo_verificacao .'</b>.',
				'AltBody' => 'Seu codigo para alterar a senha é: '. $codigo_verificacao .'.',
			],
			'devolucao_produto' => [
				'Subject' => 'Devolucao de Produto - Offer',
				'Body' => 'Infelizmente houve a devolucao de uma de suas vendas. Entre em contato com o cliente para alinhar o melhor jeito de realizar o processo. *O processo de devolucao da Offer segue o Codigo de Defesa do Consumidor, que possibilita a devolucao sem motivo no prazo de 7 dias.',
				'AltBody' => 'Infelizmente houve a devolucao de uma de suas vendas. Entre em contato com o cliente para alinhar o melhor jeito de realizar o processo. *O processo de devolucao da Offer segue o Codigo de Defesa do Consumidor, que possibilita a devolucao sem motivo no prazo de 7 dias.',
			],
		];
		...
		try {
			...
			$mail->SMTPAuth   = true;
			$mail->Username   = 'offer@offerbr.com.br';
			$mail->Password   = 'Si2p5l67ad@2';
			$mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
			$mail->Port       = 465;
			//Recipients
			$mail->setFrom('offer@offerbr.com.br', 'Offer');


			$mail->addAddress($email_hsc);

			//Content
			$mail->isHTML(true);
			$mail->Subject = $operacoes[$operacao]['Subject'];
			$mail->Body    = $operacoes[$operacao]['Body'];
			$mail->AltBody = $operacoes[$operacao]['AltBody'];

			return $mail->send();
			
		} catch (Exception $e) {

			//CRIAR UMA TRATAIVA, talvez chamar a pagina novamente dizendo ter dado um erro
